...
若 Release 尚无预编译包，可改用：
  go install github.com/x0c/agent-auto-model/cmd/agent-auto-model@${VERSION}
...
if [ ! -f "$TMP/agent-auto-model" ]; then
  die "压缩包里没有 agent-auto-model 二进制"
fi
install -m 755 "$TMP/agent-auto-model" "$BIN_DIR/agent-auto-model"
rm -f "$BIN_DIR/cursor-mode-model"

export PATH="${BIN_DIR}:$PATH"
"$BIN_DIR/agent-auto-model" install || die "二进制已装好，但 hooks 安装失败；请手动运行：agent-auto-model install"
